...
use std::fs;
use libc::{mmap, mprotect, PROT_EXEC, PROT_READ, PROT_WRITE, MAP_ANONYMOUS, MAP_FAILED, MAP_PRIVATE};
use std::{mem, ptr};
use std::mem::size_of;
...
impl<'a> JITExcutor<'a> {
    ...
    fn compile_prologue(&mut self) {
        ...
        let in_addr =  unsafe { std::mem::transmute::<*const u8, u64>(getchar as *const () as *const u8)};
        let out_addr =  unsafe { std::mem::transmute::<*const u8, u64>(putchar as *const () as *const u8)};
    ...
    fn emit_sub(&mut self, reg : u8, n : u64) {
        if n < 4095 {
            ...
            self.emit_word(base | reg as u32 | ((reg as u32) << 5) | ((n as u32) << 10)); // sub reg, reg, #<n>
        } else {
